...
export const PAGE_INTENTS = [
  ...
  "documentation",
  "exploration",    
  "exploration",      // brainstorming, free-form thinking"  // brainstorming, free-form thinking
  "tracking",         // logs, habits, metrics
  "communication",    // letters, messages, emails
  "decision_making",
  "execution",  // comparisons, pros/cons
  "other",
] as const
...
const ClassificationItem = z.object({
  ...
  secondary_categories: z.array(z.enum(PAGE_CATEGORIES)).default([]),
  // Models occasionally emit reasonable purpose words outside our enum set
  // (e.g. "report"). Accept the string and normalize downstream.
  purpose: z.string(),
  domain: z.enum(PAGE_DOMAINS),
  // Models sometimes use nearby labels like "curriculum". Accept string and normalize.
  intent: z.string(),
  audience: z.enum(PAGE_AUDIENCES),
  // Models occasionally omit topics; treat it as "no extra signal".
...
intent — what the page is FOR (functional purpose):
  self_improvement, automation, application, learning, curriculum,
  planning, execution, documentation, exploration, tracking,
  communication, decision_making, research_synthesis, other
...
Critical examples:
  ...
  5. "ML Roadmap"
       primary_category=ml, domain=engineering, intent=curriculum OR planning,
  6. "MS in Machine Learning University Shortlist"
       primary_category=us_edu, secondary_categories=[ml], domain=education,
       intent=decision_making OR planning, audience=self, purpose=comparison.
       The page is primarily about admissions choices, but it should carry
      'ml' as a secondary category because the program domain matters.
...
Rules:
  ...
  - When a page is about applying to, comparing, or evaluating ML / AI
    programs, keep 'primary_category=us_edu' and include 'ml' in
...
function normalizePurpose(raw: string): PagePurpose {
  const p = (raw || "").trim().toLowerCase()
  if (!p) return "other"
  if ((PAGE_PURPOSES as readonly string[]).includes(p)) return p as PagePurpose

  // Common near-misses we've seen from models.
  if (p === "report" || p === "doc" || p === "document" || p === "writeup") return "note"
  if (p === "plan" || p === "planning") return "roadmap"
  if (p === "log" || p === "tracker" || p === "tracking") return "journal"

  return "other"
}

function normalizeIntent(raw: string): PageIntent {
  const v = (raw || "").trim().toLowerCase()
  if (!v) return "other"
  if ((PAGE_INTENTS as readonly string[]).includes(v)) return v as PageIntent

  // Common near-misses / aliases from prompts and model habits.
  if (v === "curriculum" || v === "study" || v === "studying") return "learning"
  if (v === "research_synthesis" || v === "synthesis") return "documentation"
  if (v === "brainstorm" || v === "brainstorming" || v === "ideation") return "exploration"
  if (v === "plan" || v === "planning") return "planning"
  if (v === "track" || v === "tracking") return "tracking"
  if (v === "execute" || v === "execution") return "execution"
  if (v === "decide" || v === "decision" || v === "decision-making") return "decision_making"
...
export async function classifyPagesWithLLM(
  ...
  opts: {
    ...
    batchSize?: number
    budgetMs?: number
    providerOverride?: "auto" | "openrouter" | "openai" | "anthropic" | "gemini" | "nim" | "ollama" | "github-models" | "gateway" | null
  ...
  const envBudget = Number(process.env.VAULTMIND_CLASSIFIER_BUDGET_MS ?? "")
  const budgetDefault = Number.isFinite(envBudget) && envBudget > 0 ? envBudget : 180_000
  const provider = activeLlmProvider("linking", {
    providerOverride: opts.providerOverride ?? null,
    keys: opts.keys,
  })
  const providerBatchDefault = provider === "gemini" ? 12 : 25
  const { batchSize = providerBatchDefault, budgetMs = budgetDefault } = opts
  ...
  console.log(
    `[v0] LLM classifier (${activeLlmProvider("linking", { providerOverride: opts.providerOverride ?? null, keys: opts.keys })}): classifying ${todo.length} pages ` +
  ...
  try {
    for (let i = 0; i < todo.length; i += batchSize) {
      ...
      for (const cls of output.classifications) {
        ...
        const stored: PageClassification = {
          ...
          purpose: normalizePurpose(cls.purpose),
          domain: cls.domain,
    ...
    const hint = is401 || is403
      ? " (check OPENROUTER_API_KEY, OPENROUTER_BASE_URL, OPENROUTER_MODEL, or provider access)"
      : ""
    console.warn(
